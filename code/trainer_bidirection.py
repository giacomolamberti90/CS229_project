...
from keras.layers import Input, Dense, Dot, RepeatVector, Flatten, Permute, Multiply, Softmax, Bidirectional
...
def contrastive_loss(labels, predict):
    ...
    res = theano.tensor.split(predict, [2*model_config['output_dim'], 2*model_config['output_dim']], 2, axis=-1)
    s = res[0]
    im = res[1]
...
def train(params):
    ...
    try:
        ...
        X = Dense(2*model_config['output_dim'])(image_input)
        ...
        X_cap = Lambda(lambda x: l2norm_3d(x))(X_cap)
              
        X_attn = Bidirectional(GRU(output_dim=model_config['attn_rnn_dim'], return_sequences=True))(X)
        X_attn = Dense((1))(X_attn) # (batch_size, time_step, 1)
        X_attn = Flatten()(X_attn)
        X_attn = Softmax(axis=-1)(X_attn) # (batch_size, time_step)
        X_attn = RepeatVector(2*model_config['output_dim'])(X_attn)
        X_attn = Permute((2,1))(X_attn) # (batch_size, time_step, output_dim
        output_attn = Multiply()([X_attn, X_cap]) # (batch_size, time_step, output_dim)
        ...
        output_attn = Lambda(lambda x: compute_mean(x), output_shape=(2*model_config['output_dim'],))(output_attn)
        print(K.int_shape(output_attn))
        ...
        def eval_model():
            ...
            test_model_cap = Model(inputs=cap_input, outputs=output_attn)
        ...
        for ip in range(model_config['epoch']):
          ...
          model.save_weights('../results/pretrained_bidirectional/my_model_weights_' + str(ip) + '.h5')
